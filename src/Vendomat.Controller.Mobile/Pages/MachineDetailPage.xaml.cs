using Vendomat.Controller.Mobile.ViewModels;

namespace Vendomat.Controller.Mobile.Pages;

[QueryProperty(nameof(MachineId), "machineId")]
public partial class MachineDetailPage : ContentPage
{
    private MachineDetailViewModel ViewModel => (MachineDetailViewModel)BindingContext;

    public MachineDetailPage()
    {
        InitializeComponent();
        BindingContext = ServiceRegistry.GetRequiredService<MachineDetailViewModel>();
    }

    public string MachineId
    {
        set
        {
            if (Guid.TryParse(value, out var machineId))
            {
                _ = ViewModel.LoadAsync(machineId);
            }
        }
    }

    protected override void OnDisappearing()
    {
        ViewModel.Stop();
        base.OnDisappearing();
    }

    private async void OnBackClicked(object? sender, EventArgs e)
    {
        await Shell.Current.GoToAsync("..");
    }
}
